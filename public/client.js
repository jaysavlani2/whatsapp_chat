...
let sendLocationBtn = document.querySelector(".location");
let file_input= document.querySelector("#file-input");
let params = (new URL(window.location)).searchParams;
let name=params.get('username');
let pass=params.get('room')
let uid = getHash(pass);
...
function appendMessage(msg, type) {
    ...
    let date1 = msg.date.slice(11, 16);
    ...
    if(!regex .test(msg.message)) {
        ...
        <p>${msg.message}</p>
    `
    } else if(msg.message.startsWith("https://www.google.com/maps?q=")){
        var markup = `
        <h4>${msg.user}</h4>
        <h6>${date1}</h6>
        <a href="${msg.message}">Location📍</a>
    `
    }
    else{
        var markup = `
...
sendLocationBtn.addEventListener("click", () => {
    if (!navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(position => {
            var message= `https://www.google.com/maps?q=${position.coords.latitude},${position.coords.longitude}`
            sendMessage(message);
